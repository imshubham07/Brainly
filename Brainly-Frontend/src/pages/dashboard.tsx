...
interface Content {
  ...
  id?: string;
  link?: string;
  type?: "youtube" | "x" | "instagram" | "linkedin" | "github" | "twitter"; // Updated to include all possible types
  userId: string | { username: string }; // Could be populated with user object
  tag?: string[]; // Optional and using tag as per backend implementation
  createdAt?: string;
}
...
export function Dashboard() {
  ...
  const { contents = [] as Content[], refresh } = useContent() || { contents: [] as Content[], refresh: () => Promise.resolve() };
  ...
  const filteredContents = contents.filter((content: Content) => {
    ...
    const matchesSearch = !searchTerm ||
      ...
      (content.tag && content.tag.some((t: string) => t.toLowerCase().includes(searchTerm.toLowerCase())));
